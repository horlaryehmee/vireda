...
export function BlackHoleHeroSection({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      const requestedScale = Math.min(1, Math.max(0.4, props.current.resolution));
      // Keep the ray-marched scene near the reference preview's pixel count.
      // This preserves its framing while preventing wide desktop canvases from
      // losing the WebGL context before the first image reaches the screen.
      const pixelBudgetScale = Math.sqrt(460000 / Math.max(1, w * h));
      const scale = software
        ? 0.34
        : Math.max(0.4, Math.min(requestedScale, pixelBudgetScale));
      const sw = Math.max(2, Math.round(w * scale));
      const sh = Math.max(2, Math.round(h * scale));
      if (w === width && h === height && sw === sceneW && sh === sceneH) {
        return Boolean(scene && histA && histB && bloomA && bloomB);
      }
      width = w;
      height = h;
      sceneW = sw;
    ...
    function hasVisibleFrame() {
      ...
      const samples = [0.48, 0.58, 0.68, 0.76, 0.84, 0.92];
      gl.bindFramebuffer(gl.FRAMEBUFFER, null);
      for (const x of samples) {
        for (const y of [0.24, 0.38, 0.5, 0.62, 0.76]) {
          gl.readPixels(
          ...
          if (pixel[0] > 10 || pixel[1] > 10 || pixel[2] > 10) return true;
        }
      }
      return false;
    }
